...
`subtract` is used here because `(-1)` is treated as a *value* of `-1` not the operation 'subtract 1'.
...
Haskell uses the equals sign for assignment where you create objects, and you can be specific about the type here if you need to.
It's worth noting that Haskell uses `Num` for numbers (in general) and has more specific `Int` (integer) and `Double` (double-precision).
...
When reading that code, say "seven default gets value 7" in your head.

Code can be miserable to read on a good day, so giveyoureyesabreak and use spaces, especially either side of a binary operator (like `+` or `=`).

## Comments

Haskell will ignore any text after `--` for that line.
This allows you to write **comments**, text that is ignored by Haskell but read by other humans.
We'll sometimes include comments in examples explaining what's happening with the code.
For multi-line comments can be enclosed in `{-` and `-}`, which do not exist in R.
...
We recommend **camelCase** for variables, where the first letter is lowercase and the start of any subsequent words is capitalised, but with no separator.
For data types (which we'll cover later) use upper camel case e.g.
`DataType` or `Point`.

We'll return to names again when we discuss code style in @sec-workflow-style.

In `ghci` you can inspect an object by typing its name:
